...
(function () {
  ...
  function getShopSchedule() {
    ...
    if (users.shops && users.shops.length > 0) {
      // Look for the most recently registered shop that has a schedule
      const shopWithSched = users.shops.slice().reverse().find(s => s.schedule && (s.schedule.open || s.schedule.close));
      if (shopWithSched) {
        const s = shopWithSched.schedule;
        return {
